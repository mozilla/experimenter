...
def update_statuses(apps, schema_editor):
    ...
    ExperimentChangeLog.objects.filter(old_status="Created").update(
        old_status="Draft"
    )
    ExperimentChangeLog.objects.filter(new_status="Created").update(
        new_status="Draft"
    )

    Experiment.objects.filter(status="Pending").update(status="Review")
    ExperimentChangeLog.objects.filter(old_status="Pending").update(
        old_status="Review"
    )
    ExperimentChangeLog.objects.filter(new_status="Pending").update(
        new_status="Review"
    )

    Experiment.objects.filter(status="Launched").update(status="Live")
    ExperimentChangeLog.objects.filter(old_status="Launched").update(
        old_status="Live"
    )
    ExperimentChangeLog.objects.filter(new_status="Launched").update(
        new_status="Live"
    )
