...
class TestExperimentFilterset(MockRequestMixin, TestCase):
    def test_filters_by_multiple_types(self):
        pref = ExperimentCoreFactory.create(type=ExperimentCore.TYPE_PREF)
        addon = ExperimentCoreFactory.create(type=ExperimentCore.TYPE_ADDON)
        ExperimentCoreFactory.create(type=ExperimentCore.TYPE_GENERIC)

        filter = ExperimentFilterset(
            data=QueryDict(
                urlencode(
                    {"type": [ExperimentCore.TYPE_PREF, ExperimentCore.TYPE_ADDON]}, True
                )
            ),
    ...
    def test_filters_by_no_project_type(self):
        ...
        exp1 = ExperimentCoreFactory.create(projects=[])
        ExperimentCoreFactory.create(projects=[project1])
        ExperimentCoreFactory.create(projects=[project1])

        data = QueryDict("projects=null")

        filter = ExperimentFilterset(data=data, queryset=ExperimentCore.objects.all())
    ...
    def test_filters_by_multiple_project_type(self):
        ...
        exp1 = ExperimentCoreFactory.create(projects=[project1, project2])
        exp2 = ExperimentCoreFactory.create(projects=[project2])
        ExperimentCoreFactory.create(projects=[project3])
    ...
    def test_filters_out_archived_by_default(self):
        for i in range(3):
            ExperimentCoreFactory.create_with_status(
                ExperimentCore.STATUS_DRAFT, archived=False
            )

        for i in range(3):
            ExperimentCoreFactory.create_with_status(
                ExperimentCore.STATUS_DRAFT, archived=True
            )

        filter = ExperimentFilterset(data={}, queryset=ExperimentCore.objects.all())

        self.assertEqual(
            set(filter.qs), set(ExperimentCore.objects.filter(archived=False))
        )
    ...
    def test_allows_archived_if_True(self):
        ...
        filter = ExperimentFilterset(
            data={"archived": True}, queryset=ExperimentCore.objects.all()
        )

        self.assertEqual(set(filter.qs), set(ExperimentCore.objects.all()))
    ...
    def test_filters_by_owner(self):
        ...
        for i in range(3):
            ExperimentCoreFactory.create_with_status(
                ExperimentCore.STATUS_DRAFT, owner=owner
            )
            ExperimentCoreFactory.create_with_status(ExperimentCore.STATUS_DRAFT)

        filter = ExperimentFilterset(
            {"owner": owner.id}, queryset=ExperimentCore.objects.all()
        )

        self.assertEqual(set(filter.qs), set(ExperimentCore.objects.filter(owner=owner)))
        self.assertEqual(filter.get_owner_display_value(), str(owner))

    def test_filters_by_status(self):
        for i in range(3):
            ExperimentCoreFactory.create_with_status(ExperimentCore.STATUS_DRAFT)
            ExperimentCoreFactory.create_with_status(ExperimentCore.STATUS_REVIEW)

        filter = ExperimentFilterset(
            {"status": ExperimentCore.STATUS_DRAFT}, queryset=ExperimentCore.objects.all()
        )

        self.assertEqual(
            set(filter.qs),
            set(ExperimentCore.objects.filter(status=ExperimentCore.STATUS_DRAFT)),
        )

    def test_filters_by_firefox_version(self):

        exp_1 = ExperimentCoreFactory.create_with_variants(
            name="ExperimentCore 1",
            firefox_min_version="58.0",
            firefox_max_version="62.0",
        )
        exp_2 = ExperimentCoreFactory.create_with_variants(
            name="ExperimentCore 2",
            firefox_min_version="59.0",
            firefox_max_version="60.0",
        )
        ExperimentCoreFactory.create_with_variants(
            name="ExperimentCore 4",
            firefox_min_version="62.0",
            firefox_max_version="68.0",
        )
        exp_3 = ExperimentCoreFactory.create_with_variants(
            name="ExperimentCore 3", firefox_min_version="59.0", firefox_max_version=""
        )
        ExperimentCoreFactory.create_with_variants(
            name="ExperimentCore 5",
            firefox_min_version="54.0",
            firefox_max_version="56.0",
        )

        filter = ExperimentFilterset(
            {"firefox_version": "59.0"}, queryset=ExperimentCore.objects.all()
        )
        self.assertEqual(set(filter.qs), set([exp_1, exp_2, exp_3]))

    def test_filters_by_firefox_channel(self):
        include_channel = ExperimentCore.CHANNEL_CHOICES[1][0]
        exclude_channel = ExperimentCore.CHANNEL_CHOICES[2][0]

        for i in range(3):
            ExperimentCoreFactory.create_with_variants(firefox_channel=include_channel)
            ExperimentCoreFactory.create_with_variants(firefox_channel=exclude_channel)

        filter = ExperimentFilterset(
            {"firefox_channel": include_channel}, queryset=ExperimentCore.objects.all()
        )
        self.assertEqual(
            set(filter.qs),
            set(ExperimentCore.objects.filter(firefox_channel=include_channel)),
        )

    def test_list_filters_by_search_text(self):
        user_email = "user@example.com"

        exp_1 = ExperimentCoreFactory.create_with_status(
            random.choice(ExperimentCore.STATUS_CHOICES)[0],
            name="ExperimentCore One Cat",
            short_description="",
            slug="exp-1",
            related_work="",
        ...
        exp_2 = ExperimentCoreFactory.create_with_status(
            random.choice(ExperimentCore.STATUS_CHOICES)[0],
            name="ExperimentCore Two Cat",
            short_description="",
            slug="exp-2",
            related_work="",
        ...
        exp_3 = ExperimentCoreFactory.create_with_status(
            random.choice(ExperimentCore.STATUS_CHOICES)[0],
            name="ExperimentCore Three Dog",
            short_description="",
            slug="exp-3",
            related_work="",
    ...
    def test_filters_by_review_in_qa(self):
        ...
        filter = ExperimentFilterset(
            {"in_qa": "on"}, queryset=ExperimentCore.objects.all()
        )

        self.assertEqual(set(filter.qs), set([exp_1]))

    def test_filters_experiments_with_surveys(self):
        exp_1 = ExperimentCoreFactory.create_with_variants(survey_required=True)
        exp_2 = ExperimentCoreFactory.create_with_variants(
            survey_required=True, review_qa=False
        )
        ExperimentCoreFactory.create_with_variants(survey_required=False)

        filter = ExperimentFilterset(
            {"surveys": "on"}, queryset=ExperimentCore.objects.all()
        )

        self.assertEqual(set(filter.qs), set([exp_1, exp_2]))

    def test_filters_for_subscribed_experiments(self):
        exp_1 = ExperimentCoreFactory.create(name="ExperimentCore", slug="experiment")
    ...
    def test_filters_for_paused_experiments(self):
        exp_1 = ExperimentCoreFactory.create(
            name="ExperimentCore", is_paused=True, status=ExperimentCore.STATUS_LIVE
        )
        ExperimentCoreFactory.create()
        ExperimentCoreFactory.create()
    ...
    def test_filters_for_longrunning_experiments(self):
        ...
        exp_2 = ExperimentCoreFactory.create(
            name="ExperimentCore 2",
            firefox_min_version="64.0",
            firefox_max_version="69.0",
        )
        ExperimentCoreFactory.create(
            name="ExperimentCore 3", firefox_min_version="64.0", firefox_max_version=""
        )
        ExperimentCoreFactory.create(
            name="ExperimentCore 4",
            firefox_min_version="64.0",
    ...
    def test_filters_for_results_completed(self):
        exp1 = ExperimentCoreFactory.create(results_url="https://example.com")
        exp2 = ExperimentCoreFactory.create(results_initial="some random initial blurb")
        exp3 = ExperimentCoreFactory.create(
            results_lessons_learned="a lesson was learned"
        )
        exp4 = ExperimentCoreFactory.create(
        ...
        ExperimentCoreFactory.create()
        ExperimentCoreFactory.create(results_lessons_learned="")
    ...
    def set_up_date_tests(self):
        ...
        self.exp_1 = ExperimentCoreFactory.create_with_status(
        ...
        self.exp_2 = ExperimentCoreFactory.create_with_status(
            ExperimentCore.STATUS_DRAFT,
            name="experiment 2",
        ...
        self.exp_3 = ExperimentCoreFactory.create_with_status(
            ExperimentCore.STATUS_DRAFT,
            name="experiment 3",
        ...
        self.exp_4 = ExperimentCoreFactory.create_with_status(
            ExperimentCore.STATUS_DRAFT,
            name="experiment 4",
    ...
    def test_list_shows_all_experiments_with_start_in_range(self):
        ...
        filter = ExperimentFilterset(
            data={
                "experiment_date_field": ExperimentCore.EXPERIMENT_STARTS,
    ...
    def test_list_shows_all_experiments_with_pause_in_range(self):
        ...
        filter = ExperimentFilterset(
            data={
                "experiment_date_field": ExperimentCore.EXPERIMENT_PAUSES,
                "date_range_after": self.start_range_date,
                "date_range_before": self.end_range_date,
            }
        )
    ...
    def test_list_shows_all_experiments_with_end_in_range(self):
        ...
        filter = ExperimentFilterset(
            data={
                "experiment_date_field": ExperimentCore.EXPERIMENT_ENDS,
                "date_range_after": self.start_range_date,
                "date_range_before": self.end_range_date,
            }
        )
