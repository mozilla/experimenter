...
class TestExperimentCoreManager(TestCase):
    def test_queryset_annotated_with_latest_change(self):
        now = timezone.now()
        experiment1 = ExperimentCoreFactory.create_with_variants()
        experiment2 = ExperimentCoreFactory.create_with_variants()
        ...
        self.assertEqual(
            list(ExperimentCore.objects.order_by("-latest_change")),
        ...
        ExperimentChangeLogFactory.create(
            ...
            new_status=ExperimentCore.STATUS_REVIEW,
        )
...
class TestExperimentCoreModel(TestCase):
    def test_get_absolute_url(self):
        experiment = ExperimentCoreFactory.create(slug="experiment-slug")
    ...
    def test_bugzilla_url_returns_url_when_bugzilla_id_is_set(self):
        experiment = ExperimentCoreFactory.create_with_status(
            ExperimentCore.STATUS_DRAFT, bugzilla_id="1234"
        )
        self.assertEqual(
    ...
    def test_monitoring_dashboard_url_is_none_when_experiment_not_begun(self):
        experiment = ExperimentCoreFactory.create(
            slug="experiment",
            status=ExperimentCore.STATUS_DRAFT,
    ...
    def test_has_external_urls_is_true_when_bugzilla_url_is_set(self):
        experiment = ExperimentCoreFactory.create(bugzilla_id="1234")
        self.assertTrue(experiment.has_external_urls)

    def test_has_external_urls_is_true_when_monitoring_dashboard_url_is_set(self):
        experiment = ExperimentCoreFactory.create(
            status=ExperimentCore.STATUS_LIVE, recipe_slug="recipe-slug"
        )
        self.assertTrue(experiment.has_external_urls)
    ...
    def test_should_use_normandy_false_for_generic(self):
        experiment = ExperimentCoreFactory.create(type=ExperimentCore.TYPE_GENERIC)
        self.assertFalse(experiment.should_use_normandy)

    def test_should_use_normandy_true_for_pref(self):
        experiment = ExperimentCoreFactory.create(type=ExperimentCore.TYPE_PREF)
        self.assertTrue(experiment.should_use_normandy)

    def test_should_use_normandy_true_for_addon(self):
        experiment = ExperimentCoreFactory.create(type=ExperimentCore.TYPE_ADDON)
        self.assertTrue(experiment.should_use_normandy)
    ...
    def test_is_pref_rollout(self):
        experiment = ExperimentCoreFactory.create(
            type=ExperimentCore.TYPE_ROLLOUT, rollout_type=ExperimentCore.TYPE_PREF
        )
        self.assertTrue(experiment.is_pref_rollout)

    def test_is_addon_rollout(self):
        experiment = ExperimentCoreFactory.create(
            type=ExperimentCore.TYPE_ROLLOUT, rollout_type=ExperimentCore.TYPE_ADDON
        )
        self.assertTrue(experiment.is_addon_rollout)

    def test_normandy_recipe_json_serializes_pref_study(self):
        experiment = ExperimentCoreFactory.create_with_status(ExperimentCore.STATUS_SHIP)
    ...
    def test_compute_end_date_accepts_None_start_date(self):
        experiment = ExperimentCoreFactory.create_with_variants(proposed_start_date=None)
    ...
    def test_enrollment_ending_soon(self):
        experiment_1 = ExperimentCoreFactory.create_with_variants(
        ...
        experiment_2 = ExperimentCoreFactory.create_with_variants(
            proposed_start_date=datetime.date.today(),
            proposed_duration=20,
    ...
    def test_rollout_dates_low_risk_playbook(self):
        experiment = ExperimentCoreFactory.create(
            ...
            rollout_playbook=ExperimentCore.ROLLOUT_PLAYBOOK_LOW_RISK,
    ...
    def test_rollout_dates_high_risk_playbook(self):
        experiment = ExperimentCoreFactory.create(
            type=ExperimentCore.TYPE_ROLLOUT,
            rollout_playbook=ExperimentCore.ROLLOUT_PLAYBOOK_HIGH_RISK,
            proposed_start_date=datetime.date(2020, 1, 1),
        )
        self.assertEqual(
            experiment.rollout_dates,
    ...
    def test_rollout_dates_marketing_playbook(self):
        experiment = ExperimentCoreFactory.create(
            type=ExperimentCore.TYPE_ROLLOUT,
            rollout_playbook=ExperimentCore.ROLLOUT_PLAYBOOK_MARKETING,
            proposed_start_date=datetime.date(2020, 1, 1),
        )
        self.assertEqual(
            experiment.rollout_dates,
    ...
    def test_rollout_dates_custom_playbook(self):
        experiment = ExperimentCoreFactory.create(
            type=ExperimentCore.TYPE_ROLLOUT,
            rollout_playbook=ExperimentCore.ROLLOUT_PLAYBOOK_CUSTOM,
            proposed_start_date=datetime.date(2020, 1, 1),
        )
    ...
    def test_enrollment_is_complete(self):
        experiment = ExperimentCoreFactory.create_with_status(
            target_status=ExperimentCore.STATUS_LIVE, is_paused=True
        )
        self.assertTrue(experiment.is_enrollment_complete)

    def test_enrollment_is_not_complete(self):
        experiment = ExperimentCoreFactory.create_with_status(
            target_status=ExperimentCore.STATUS_LIVE, is_paused=False
        )
    ...
    def test_experiment_is_editable_as_draft(self):
        experiment = ExperimentCoreFactory.create_with_status(ExperimentCore.STATUS_DRAFT)
    ...
    def test_experiment_is_not_begun(self):
        statuses = (ExperimentCore.STATUS_DRAFT, ExperimentCore.STATUS_REVIEW)
    ...
    def test_experiment_is_begun(self):
        for status in ExperimentCore.STATUS_LIVE, ExperimentCore.STATUS_COMPLETE:
            experiment = ExperimentCoreFactory.create_with_status(status)
            self.assertTrue(experiment.is_begun)

    def test_overview_is_not_complete_when_not_saved(self):
        experiment = ExperimentCoreFactory.build()
    ...
    def test_population_is_complete_for_rollout_without_population_percent(self):
        experiment = ExperimentCoreFactory.create(
            type=ExperimentCore.TYPE_ROLLOUT, population_percent=0.0
        )
        self.assertTrue(experiment.completed_population)

    def test_design_is_not_complete_when_defaults_set(self):
        experiment = ExperimentCoreFactory.create(design=ExperimentCore.DESIGN_DEFAULT)
        self.assertFalse(experiment.completed_design)

    def test_design_is_complete_when_design_set(self):
        experiment = ExperimentCoreFactory.create(design="Design")
    ...
    def test_completed_addon_is_not_complete_when_release_url_not_set_for_branched(self):
        experiment = ExperimentCoreFactory.create_with_variants(
            type=ExperimentCore.TYPE_ADDON, is_branched_addon=True
        )
    ...
    def test_objectives_is_not_complete_with_still_default(self):
        experiment = ExperimentCoreFactory.create(
            objectives=ExperimentCore.OBJECTIVES_DEFAULT,
            analysis=ExperimentCore.ANALYSIS_DEFAULT,
        )
    ...
    def test_is_high_risk_if_any_risk_questions_are_true(self):
        ...
        for true_risk_field in risk_fields:
            ...
            experiment = ExperimentCoreFactory.create(**instance_risk_fields)
    ...
    def test_completed_all_sections_false_for_branchedadd_exp_without_url(self):
        experiment = ExperimentCoreFactory.create_with_variants(
            is_branched_addon=True, type=ExperimentCore.TYPE_ADDON
        )
        variant = experiment.variants.first()
        variant.addon_release_url = None
        variant.save()
    ...
    def test_completed_all_sections_false_for_generic_without_design(self):
        experiment = ExperimentCoreFactory.create_with_status(
            ExperimentCore.STATUS_REVIEW,
            type=ExperimentCore.TYPE_GENERIC,
            design=ExperimentCore.DESIGN_DEFAULT,
        )
        self.assertFalse(experiment.completed_all_sections)
    ...
    def test_experiment_is_not_archivable(self):
        ...
        experiment2 = ExperimentCoreFactory.create_with_status(
            target_status=ExperimentCore.STATUS_LIVE
        )
        self.assertFalse(experiment.is_archivable)
    ...
    def test_use_branched_addon_serializer_returns_true_for_addon_and_greater_version(
        ...
        experiment = ExperimentCoreFactory(
            type=ExperimentCore.TYPE_ADDON, firefox_min_version="70.0"
        )
    ...
    def test_use_branched_addon_serializer_returns_false_for_addon_and_lower_version(
        ...
        experiment = ExperimentCoreFactory(
            type=ExperimentCore.TYPE_ADDON, firefox_min_version="66.0"
        )
    ...
    def test_experiment_firefox_channel_sort_does_sorting(self):
        ExperimentCoreFactory.create(firefox_channel=ExperimentCore.CHANNEL_NIGHTLY)
        ExperimentCoreFactory.create(firefox_channel=ExperimentCore.CHANNEL_RELEASE)
        ExperimentCoreFactory.create(firefox_channel=ExperimentCore.CHANNEL_BETA)
        ExperimentCoreFactory.create(firefox_channel="")
        sorted_experiments = (
            ExperimentCore.objects.annotate(
                firefox_channel_sort=ExperimentCore.firefox_channel_sort()
            )
        ...
        self.assertEqual(
            ...
            [
                ExperimentCore.CHANNEL_UNSET_ORDER,
                ExperimentCore.CHANNEL_NIGHTLY_ORDER,
                ExperimentCore.CHANNEL_BETA_ORDER,
                ExperimentCore.CHANNEL_RELEASE_ORDER,
            ],
        )

    def test_should_have_total_enrolled_true(self):
        experiment = ExperimentCoreFactory(type=ExperimentCore.TYPE_PREF)
    ...
    @parameterized.expand(
        [
            (False, ExperimentCore.TYPE_ADDON),
            (False, ExperimentCore.TYPE_GENERIC),
            (False, ExperimentCore.TYPE_PREF),
            (False, ExperimentCore.TYPE_ROLLOUT),
            (True, ExperimentCore.TYPE_MESSAGE),
        ]
    )
    def test_should_have_telemetry_event(self, expected, type):
        experiment = ExperimentCoreFactory(type=type)
        self.assertEqual(experiment.should_have_telemetry_event, expected)

    def test_display_platforms(self):
        experiment_1 = ExperimentCoreFactory(platforms=["All Windows", "All Mac"])
        experiment_2 = ExperimentCoreFactory(
            platforms=["All Windows", "All Mac", "All Linux"]
        )
        experiment_3 = ExperimentCoreFactory(windows_versions=["Windows 8"])

        self.assertEqual(
            experiment_1.display_platforms_or_versions, "All Windows, All Mac"
        )
        self.assertEqual(experiment_2.display_platforms_or_versions, "All Platforms")
        self.assertEqual(experiment_3.display_platforms_or_versions, "Windows 8")
    ...
    def test_clone(self):
        ...
        experiment = ExperimentCoreFactory.create_with_variants(
            ...
            owner=user_1,
            bugzilla_id="4455667",
            pref_type=ExperimentCore.TYPE_ADDON,
            ...
            review_science=True,
            review_ux=True,
            firefox_min_version=ExperimentCore.VERSION_CHOICES[1][0],
            firefox_max_version="",
        ...
        cloned_experiment = ExperimentCore.objects.filter(name="best experiment").get()

        self.assertTrue(cloned_experiment.parent, experiment.id)
        self.assertIn(experiment, cloned_experiment.related_to.all())
        self.assertEqual(cloned_experiment.status, ExperimentCore.STATUS_DRAFT)
        self.assertEqual(
        ...
        self.assertEqual(cloned_experiment.pref_type, ExperimentCore.TYPE_ADDON)
        self.assertEqual(cloned_experiment.proposed_start_date, None)
        self.assertEqual(cloned_experiment.owner, user_2)
        self.assertEqual(
            cloned_experiment.firefox_min_version, ExperimentCore.VERSION_CHOICES[1][0]
        )
    ...
    def test_variantPreferences_are_included_in_clone(self):
        user = UserFactory.create()
        experiment = ExperimentCoreFactory.create(is_multi_pref=True)
        ...
        cloned_experiment = ExperimentCore.objects.get(name="cloned experiment")
...
class TestVariantPreferences(TestCase):
    def setUp(self):
        ...
        self.pref.pref_type = ExperimentCore.PREF_TYPE_BOOL
        self.pref.pref_value = "true"
        self.pref.pref_branch = ExperimentCore.PREF_BRANCH_DEFAULT
    ...
    def test_unique_pref_name_constraint_is_violated(self):
        ...
        pv2.pref_type = ExperimentCore.PREF_TYPE_BOOL
        pv2.pref_value = "false"
        pv2.pref_branch = ExperimentCore.PREF_BRANCH_DEFAULT
    ...
    def test_is_json_string_type_returns_true_for_json_string_type(self):
        self.pref.pref_type = ExperimentCore.PREF_TYPE_JSON_STR
        self.pref.save()
...
class TestExperimentComments(TestCase):
    def test_manager_returns_sections(self):
        ...
        risk_comment = ExperimentCommentFactory.create(
            experiment=experiment, section=ExperimentCore.SECTION_RISKS
        )
        testing_comment = ExperimentCommentFactory.create(
            experiment=experiment, section=ExperimentCore.SECTION_TESTING
        )
        self.assertIn(
...
class TestExperimentBucketNamespace(TestCase):
    def test_empty_namespace_creates_namespace_and_bucket_range(self):
        ...
        self.assertEqual(bucket.namespace.total, ExperimentCore.BUCKET_TOTAL)
        self.assertEqual(
            bucket.namespace.randomization_unit, ExperimentCore.BUCKET_RANDOMIZATION_UNIT
        )
