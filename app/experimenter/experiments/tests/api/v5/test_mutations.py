...
class TestUpdateExperimentMutationSingleFeature(
    ...
    def test_swap_experiment_branches(self):
        user_email = "user@example.com"
        feature = NimbusFeatureConfigFactory(
            schema="{}", application=NimbusExperiment.Application.FENIX
        )
        ...
        response = self.query(
            ...
            variables={
                "input": {
                    "id": experiment.id,
                    "featureConfigId": feature.id,
                    "referenceBranch": swap_reference_branch_data,
                    "treatmentBranches": swap_treatment_branches_data,
                    "changelogMessage": "test changelog message",
                }
            },
            headers={settings.OPENIDC_EMAIL_HEADER: user_email},
        ...
        self.assertEqual(experiment.branches.count(), 2)
        self.assertEqual(
            experiment.reference_branch.name, swap_reference_branch_data["name"]
        )
        self.assertEqual(experiment.reference_branch.feature_values.get().enabled, True)
        self.assertEqual(experiment.reference_branch.feature_values.get().value, "")
        treatment_branch = experiment.treatment_branches[0]
        self.assertEqual(treatment_branch.name, swap_treatment_branches_data[0]["name"])
        self.assertEqual(treatment_branch.feature_values.get().enabled, True)
        self.assertEqual(
            treatment_branch.feature_values.get().value,
            swap_treatment_branches_data[0]["featureValue"],
        )
