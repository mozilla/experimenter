...
class NimbusExperimentManager(models.Manager["NimbusExperiment"]):
    ...
    def latest_changed(self):
        return (
            super()
            .get_queryset()
            .annotate(latest_change=Max("changes___updated_date_time"))
...
class NimbusExperiment(NimbusConstants, TargetingConstants, FilterMixin, models.Model):
    ...
    def start_date(self):
        ...
        if self.is_started:
            if (
                ...
                self._start_date = start_changelog._updated_date_time.date()
                self.save()
                return self._start_date
    ...
    def end_date(self):
        ...
        if self.status == self.Status.COMPLETE:
            if (
                ...
                self._end_date = end_changelog._updated_date_time.date()
                self.save()
                return self._end_date
    ...
    def computed_enrollment_days(self):
        ...
        if self.is_paused:
            if paused_changelogs := [
                ...
                paused_change = sorted(paused_changelogs, key=lambda c: c._updated_date_time)[-1]
                self._enrollment_end_date = paused_change._updated_date_time.date()
                self.save()
                return (paused_change._updated_date_time.date() - self.start_date).days
    ...
    def should_timeout(self):
        review_expired = (
            timezone.now() - self.changes.latest_change()._updated_date_time
...
class NimbusChangeLogManager(models.Manager["NimbusChangeLog"]):
    def latest_change(self):
        return self.all().order_by("-_updated_date_time").first()
...
class NimbusChangeLog(FilterMixin, models.Model):
    ...
    _updated_date_time = models.DateTimeField(default=current_datetime)
    ...
    def __str__(self):
        return self.message or (
            ...
            f"by {self.changed_by} on {self._updated_date_time}"
        )
