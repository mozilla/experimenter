...
const CopyableURL: React.FC<CopyableURLProps> = ({
  ...
  return (
    <code className="click-to-copy" onClick={() => clipboard.writeText(url)}>
      {url}
    </code>
  );
};
...
export const PreviewURL: React.FC<
  ...
  >
> = ({ slug: experimentSlug, referenceBranch, treatmentBranches }) => {
  const [selection, setSelection] = React.useState(referenceBranch?.slug || "");
  ...
  return (
    <>
      ...
      <p>
        <b>Click to copy the link</b> you want to preview and paste it in your
        browser. You need to enable <code>nimbus.debug</code> pref before.
      </p>
      <form data-testid="preview-url-form">
      ...
      <h6>For LIVE Experiments:</h6>
      <p>{CopyableURL({ experimentSlug, branch: selection })}</p>
      <h6>For Experiments in PREVIEW:</h6>
      <p>
        {CopyableURL({
          experimentSlug,
          branch: selection,
