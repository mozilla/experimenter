...
class NimbusExperiment(NimbusConstants, TargetingConstants, FilterMixin, models.Model):
    ...
    def metric_areas(self, analysis_basis, segment, reference_branch, window="overall"):
        metric_areas = {}
        ...
        for slug in self.primary_outcomes + self.secondary_outcomes:
        ...
        def metric_is_notable(slug, group):
        ...
        for metrics in metric_areas.values():
            for metric in metrics:
                if (
                    metric_is_notable(metric["slug"], metric["group"])
