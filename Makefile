...
check: test_build
	$(COMPOSE_TEST) run app sh -c '$(WAIT_FOR_DB) ${PARALLEL} "$(PYTHON_CHECK_MIGRATIONS)" "$(CHECK_DOCS)" "$(BLACK_CHECK)" "${PY_IMPORT_CHECK}" "$(FLAKE8)" "$(ESLINT_CORE)" "$(ESLINT_RAPID)" "$(TYPECHECK_RAPID)" "$(PYTHON_TEST)" "$(JS_TEST_CORE)" "$(JS_TEST_RAPID)"'
...
code_format: compose_build
	$(COMPOSE) run app sh -c "$(BLACK_FIX)&&${PY_IMPORT_SORT}&&$(ESLINT_FIX_CORE)&&$(ESLINT_FIX_RAPID)"
