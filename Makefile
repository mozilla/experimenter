...
integration_test_nimbus: integration_build
	$(COMPOSE_INTEGRATION) up -d; MOZ_HEADLESS=1 $(COMPOSE_INTEGRATION) run firefox sh -c "sudo chmod -R a+rwx /code/app/tests/integration/;tox -c app/tests/integration -e integration-test-nimbus $(TOX_ARGS) -- --html=report.html --self-contained-html  $(PYTEST_ARGS)"
